...
import (
	...
	"github.com/retr0-kernel/worker-mesh/ent/migrate"
...
func (c *Client) Intercept(interceptors ...Interceptor) {
	...
	c.WorkerNode.Intercept(interceptors...)
}
...
// UpdateOne returns an update builder for the given entity.
func (c *WorkerNodeClient) UpdateOne(wn *WorkerNode) *WorkerNodeUpdateOne {
	mutation := newWorkerNodeMutation(c.config, OpUpdateOne, withWorkerNode(wn))
	return &WorkerNodeUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *WorkerNodeClient) UpdateOneID(id int) *WorkerNodeUpdateOne {
...
// DeleteOne returns a builder for deleting the given entity.
func (c *WorkerNodeClient) DeleteOne(wn *WorkerNode) *WorkerNodeDeleteOne {
	return c.DeleteOneID(wn.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *WorkerNodeClient) DeleteOneID(id int) *WorkerNodeDeleteOne {
...
func (c *WorkerNodeClient) Get(ctx context.Context, id int) (*WorkerNode, error) {
	return c.Query().Where(workernode.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *WorkerNodeClient) GetX(ctx context.Context, id int) *WorkerNode {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
type (
	hooks struct {
		Job, WorkerNode []ent.Hook
	}
	inters struct {
		Job, WorkerNode []ent.Interceptor
	}
)
